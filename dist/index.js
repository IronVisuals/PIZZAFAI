...
function ensureDirs() {
    if (!fs.existsSync(pastaAtivos))
        fs.mkdirSync(pastaAtivos, { recursive: true });
...
function carregarProdutos() {
    ...
    for (const linha of linhas) {
        ...
        if (parseInt(id) >= proximoIdProduto)
            proximoIdProduto = parseInt(id) + 1;
    }
}
...
function listarProdutosPorCategoria(cat) {
...
function cadastrarProdutoFlow() {
    ...
    const mapa = { 1: "PIZZA", 2: "BEBIDA", 3: "SOBREMESA", 4: "OUTRO" };
    ...
    const novo = { id: proximoIdProduto++, nome, categoria, preco, ativo: true };
    produtos.push(novo);
    salvarProduto(novo);
...
function carregarVendas() {
    ...
    for (const linha of linhas) {
        ...
        if (idNum >= proximoIdVenda)
            proximoIdVenda = idNum + 1;
    }
}
function salvarVenda(v) {
    if (!fs.existsSync(vendasPath)) {
        fs.writeFileSync(vendasPath, "ID;DataISO;Item;Categoria;Quantidade;ValorUnitario;ValorTotal\n", "utf-8");
    }
    fs.appendFileSync(vendasPath, `${v.id};${v.dataISO};${v.item};${v.categoria};${v.quantidade};${v.valorUnitario};${v.valorTotal}\n`, "utf-8");
}
// ---------- RELATÓRIOS ----------
function relatorioPizzasPorDia() {
    const linhas = lerCSV(vendasPath).slice(1);
    const mapa = new Map(); // "YYYY-MM-DD" -> qtd
...
function relatorioPizzasPorMes() {
    const linhas = lerCSV(vendasPath).slice(1);
    const mapa = new Map(); // "YYYY-MM" -> qtd
    for (const l of linhas) {
...
function fluxoPedido() {
    ...
    if (pizzas.length === 0) {
        console.log("Não há pizzas cadastradas.");
        return;
    }
    console.log("\nPIZZAS DISPONÍVEIS:");
